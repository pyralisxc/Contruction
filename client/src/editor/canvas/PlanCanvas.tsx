...
import { SelectionHandle, SnapTarget } from '../types'
    ...
    <div className={`plan-wrap tool-${activeTool}`}>
      <div className="plan-head">
        ...
        <div className="draw-options">
          ...
          <label>
            <span>D</span>
            <input value={typedDepth} inputMode="decimal" placeholder="ft" onChange={(event) => setTypedDepth(event.target.value)} disabled={activeTool !== 'drawFloor' && activeTool !== 'drawRoof'} />
          </label>
        </div>
      ...
      >
        ...
        <g transform={viewportTransform(planViewport)}>
          ...
          {visibleLayers.roof && roofs.map((roof) => <RoofShape key={roof.id} roof={roof} selectedId={selectedId} onSelect={onSelect} />)}
          {visibleLayers.floors && floors.map((floor) => <FloorShape key={floor.id} floor={floor} selectedId={selectedId} onSelect={onSelect} />)}
          ...
          {visibleLayers.walls && walls.map((wall) => <WallShape key={wall.id} wall={wall} selectedId={selectedId} onSelect={onSelect} />)}
...
function FloorShape({ floor, selectedId, onSelect }: { floor: FloorElement; selectedId: string | null; onSelect: (id: string) => void }) {
  return (
    <polygon
...
function RoofShape({ roof, selectedId, onSelect }: { roof: RoofElement; selectedId: string | null; onSelect: (id: string) => void }) {
  return (
    <polygon
...
function WallShape({ wall, selectedId, onSelect }: { wall: WallElement; selectedId: string | null; onSelect: (id: string) => void }) {
...
function SelectionHandles({
  ...
  if (selected.type === 'floor') return <RectHandles id={selected.id} points={selected.polygon} prefix="floor" startHandleDrag={startHandleDrag} />
  if (selected.type === 'roof') return <RectHandles id={selected.id} points={selected.footprint} prefix="roof" startHandleDrag={startHandleDrag} />
  if (selected.type === 'wall') {
    return (
      <>
...
function RectHandles({
  ...
  const corners: { handle: SelectionHandle; point: Point2 }[] = [
    { handle: `${prefix}-nw`, point: { x: bounds.minX, y: bounds.minY } },
    { handle: `${prefix}-ne`, point: { x: bounds.maxX, y: bounds.minY } },
    { handle: `${prefix}-se`, point: { x: bounds.maxX, y: bounds.maxY } },
    { handle: `${prefix}-sw`, point: { x: bounds.minX, y: bounds.maxY } },
  ]
  return (
    <>
      {corners.map((corner) => (
        <Handle key={corner.handle} point={corner.point} onPointerDown={(event) => startHandleDrag(event, id, corner.handle)} />
      ))}
...
function Handle({ point, kind = 'resize', onPointerDown }: { point: Point2; kind?: 'resize' | 'move'; onPointerDown: (event: React.PointerEvent<SVGCircleElement>) => void }) {
  const svg = toSvg(point)
  return <circle className={`selection-handle handle-${kind}`} cx={svg.x} cy={svg.y} r={kind === 'move' ? 6 : 5} onPointerDown={onPointerDown} />
}
